...
###
TimePoints <- c("T0", "T024", "T048", "T072", "T120", "T168")		## how many points in time does the experiment cover? Provide a label for each time-point in the format c("T0", "T1", "T2", ...) or leave at 'FALSE' for no time-splitting
...
###
sl_classes <- "md_d_2"   				## the name of the xlsx file in the metadata folder holding the class structure. Leave at 'def' to get the default from the settings (parameter 'fn_class_structure').
###
Repls <- 6								## how many replications for each sample?
Group <- c("Cont", "ChemA", "ChemB")	## additional groups to split the above generated classes into, like e.g. experiment and control
