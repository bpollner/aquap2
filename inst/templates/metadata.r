...
tempHumLog <- "def"						## If data from temperatur and rel.humidity logger should be imported and aligned to a timestamp in the dataset. Leave at 'def' to get the default from the settings (parameter 'imp_use_TRH_logfile').  If a value other than "def" is provided in the argument "trhLog" in the function "getFullData", this is overriding the value of "tempHumLog" in the metadata file. Please refer to the help for ?getFullData for possible values.
