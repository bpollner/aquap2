...
expName <- "4gr 1class4"						## the name of the experiment
...
###
columnNamesL1 <- c("C_waterName")		## please note that there is a special prefix for class- and numerical variables: the default is "C_" for class-variables and "Y_" for numerical variables
...
L1  <- list(list(c("Pinoir", "Aquavit", "Fountainbleu", "Ramedi"))) 	## Please look at the vignette 'Examples for Experiment-Design' for more information on the use of the arguments L1 and L2.
L2  <- list(list(c(rep("-", 4))))
###
Repls <- 5								## how many replications for each sample?
Group <- c("TreatmentA", "TreatmentB", "TreatmentC", "Cont")		## additional groups to split the above generated classes into, like e.g. experiment and control
