...
expName <- "1gr 3class4-2-3 3subclass"						## the name of the experiment
...
###
columnNamesL1 <- c("C_waterName", "C_waterCondition", "Y_NaCl_mg")		## please note that there is a special prefix for class- and numerical variables: the default is "C_" for class-variables and "Y_" for numerical variables
columnNamesL2 <- c("C_source", "Y_sparkLevel", "C_NaClSource")					## "DELETE" is the default character for those columns that, after the generation of the sample list, should be omitted
L1  <- list(list("Pinoir", "Aquavit", "Fountainbleu", "Ramedi"), list("sparkling", "still"), list(as.character(c(5, 15, 30))) ) 	## Please look at the vignette 'Examples for Experiment-Design' for more information on the use of the arguments L1 and L2.
L2  <- list(list(c("A", "D", "CH"), c("A", "D", "CH"), c("CH", "F", "N"), c("CH", "F", "N", "D", "A")), list(as.character(c(12,24,36)), as.character(c(0))), list(c("Merck", "Pfizer", "Bayer"), c("Pfizer", "Bayer", "AstraS"), c("AstraS", "Sandoz")) )
###
