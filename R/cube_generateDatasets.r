...
checkLoadNoiseFile <- function(header, ap, md, noiseFile) {
	useNoise <- ap$dpt$noise$useNoise
	if (useNoise) {
		if (all(noiseFile == "def")) {
			noiseFile <- .ap2$stn$noi_noiseDataFilename
			nfMeta <- md$meta$noiseFile
		}
		...
		if (.ap2$stn$noi_forceExclusionOfOutliers) {
			...
			if (!cnOutlier %in% cns) {
				stop(paste0("You need to have outliers detected in the whole noise-dataset. Please re-import the noise data (function 'gfd') and make sure to have the argument 'dol' or it´s default in the settings.r file (parameter 'imp_flagOutliers') set to TRUE.\nPlease refer to ?gfd for more information.\n(You can deactivate the forced exclusion of outliers of the noise-data file in the settings at the argument 'noi_forceExclusionOfOutliers'.)"), call.=FALSE)
			}
			 noiDataset <- ssc_s(noiDataset, cnOutlier, FALSE) # only keep the non-outliers
		} # end force exclusion	
		## now check age-difference		
		...
		if (.ap2$stn$noi_forceTimeDifferenceCheck & !is.null(tsData) & !is.null(tsNoise)) {
			...
			if (diffT > .ap2$stn$noi_acceptTimeDiffDays) {
				stop(paste0("Too Old!!"), call.=FALSE)
			}
		}
		return(noiDataset)
	} else { #
...
#' to use this as source for the noise data. Please see 
#' \code{\link{noise_procedures}} for more information.
...
#' @export
gdmm <- function(dataset, ap=getap(), noiseFile="def") {
	autoUpS(); ap;
	...
	md <- getMetadata(dataset)
	##
	noiseDataset <- checkLoadNoiseFile(dataset$header, ap, md, noiseFile) # only if noise is added; if not returns NULL
	noi_calculateNoiseDistribution(noiseDataset) # only if noise: calculate noise distribution, save as global variable in .ap2
	##
