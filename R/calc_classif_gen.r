...
makeOuterSplitList <- function(dataset, percTest, classOn, stnLoc=getstn()) {
	if (percTest <= 0) {
		nrOuterLoops <- 1
	} else {
...
#	cns <- colnames(getDataset(cube[[1]])$header) # all datasets must have the same header structure, so just take the first
	...
	###
	if (all(toxls == "def")) {
		toxls <- stn$cl_indepPred_exportToExcel
	}
	...
	#
	if (all(info == "def")) {
		info <- stn$cl_indepPred_showInfo
	}
	...
	#
	if (all(confirm == "def")) {
		confirm <- stn$cl_indepPred_confirm
	}
